...
export default function SeeOrderForm({ title, open, onCancel, id }) {
  ...
  const getInitalValues = async () => {
    if (id) {
      ...
      form.setFieldValue("gatherLocationSend", transLocal.gatherTrans.nameGather);
    } else {
      form.resetFields();
    }
  };
  ...
  return (
    <Modal open={open} title={title} onCancel={onCancel} onOk={handleSubmit}>
      <Form form={form} layout="vertical" onFinish={handleFinish}>
        ...
        <Form.Item
          name="gatherLocationSend"
          label="Tên điểm tập kết gửi đơn hàng"
          rules={[{ required: true }]}
        >
          {/* <Select
            showSearch
